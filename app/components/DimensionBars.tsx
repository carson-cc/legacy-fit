import { computeCompositeDimensions } from '@/lib/scoring'
...
export default function DimensionBars({ coords, size = 'sm' }: Props) {
  const composite = computeCompositeDimensions(coords)
      ...
      {DIMS.map(({ key, label }) => (
        <div key={key} className="flex items-center gap-3">
          ...
          <div className={`${trackW} ${barH} bg-[#1a1a1a] rounded-full overflow-hidden`}>
            <div className={`${barH} bg-white rounded-full`} style={{ width: `${composite[key]}%` }} />
          </div>
        </div>
      ))}
    </div>
